...
```text
https://ijudge.it.kmitl.ac.th/submissions/602050/overview
```
...
```text
หาแถวก่อนและหาลำดับในแถวและดูว่าลำดับในแถวเป็นเลขคู้หรือเลขคี่
Input: จำนวนเต็ม N (1 < N < 100,000)
Output: จำนวนกำแพงน้อยที่สุดที่ต้องพัง (ถ้าN= ตอบ 0)
Constraints: N มีค่าได้ถึง 100,000
```
...
1 รับค่า N
2 ถ้า N == 1
3 ถ้าจริง ให้พิมพ์ 0
3: คำนวณหาแถว r = math.ceil(math.sqrt(N)) #รากที่สองของNปัดขึ้นตลอด
4: คำนวณหาลำดับห้องในแถว k = N - (r - 1) ** 2
5: ตรวจสอบว่า k % 2 == 0 หรือไม่
6: ถ้าจริง (เป็นสามเหลี่ยมคว่ำ) ให้พิมพ์ 2 * r - 3
7: ถ้าไม่จริง (เป็นสามเหลี่ยมหงาย) ให้พิมพ์ 2 * r - 2
```
...
หาแถวก่อนและคำนวณหาลำดับห้องในแถวและดูว่าลำดับในแถวเป็นเลขคู่หรือคี่ถ้าเป็ยเลขคู่ให้ 2 * r - 3 ถ้าเป็นเลขคี่ให้2 * r - 2
```
