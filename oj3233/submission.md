...
```text
โจทย์นี้ให้ตรวจว่า สลากกินแบ่งที่เราซื้อถูกรางวัลหรือไม่
input:
บรรทัดที่ 1: ผลสลากที่รัฐบาลประกาศ
บรรทัดที่ 2: สลากที่เราซื้อ
output:
ให้แสดงจำนวนเงินรางวัลที่ได้รับ
Constraints:
เลขสลากมี 5 หลัก เสมอ
ต้องพิจารณา รางวัลที่มากที่สุดเพียงรางวัลเดียว
```
...
```text
Step 1: รับข้อมูลผลสลากที่รัฐบาลประกาศ และสลากที่ซื้อมา
Step 2: เปรียบเทียบว่าตัวอักษรของทั้งสองสลากตรงกันหรือไม่
Step 3: เปรียบเทียบเลขสลาก 5 หลักว่าตรงกันทั้งหมดหรือไม่
Step 4: เปรียบเทียบเลขท้าย 3 ตัวว่าตรงกันหรือไม่
Step 5: เปรียบเทียบเลขท้าย 2 ตัวว่าตรงกันหรือไม่
Step 6: ตรวจเงื่อนไขรางวัลจาก รางวัลที่มีมูลค่าสูงสุดไปต่ำสุด
Step 7: ถ้าตรงเงื่อนไขใด ให้แสดงเงินรางวัลนั้นและจบการทำงานทันที
Step 8: ถ้าไม่ตรงกับเงื่อนไขใดเลย ให้แสดง 0
```
...
```text
เหมือนกับแผนแรก เพราะใช้การเปรียบเทียบข้อมูลทีละเงื่อนไข โดยเริ่มจากรางวัลที่มีมูลค่าสูงสุดก่อน
```
...
```text
เลข 5 หลักตรง แต่ตัวอักษรไม่ตรง
```
